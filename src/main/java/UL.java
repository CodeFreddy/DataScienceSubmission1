...
public class UL {
    ...
    public UL(Map<String, String> queriesStr, int resultsNum, String indexPath, String outputPath, String outputName)
    {
        INDEX_DIR = indexPath;
    ...
    private HashMap<String, Float> getRanked(String queryStr) throws IOException{
        ...
        try {
            ...
            for(int i = 0; i < hits.length; i++)
            {
                ...
                //size of vocabulary
                int sizeOfVoc = getSizeOfVocabulary(unigramList);
                ...
                for(String term : queryList)
                {
                    ...
                    int tf = countExactTermFreqInList(term, unigramList);
                    float p = laplaceSmoothing(tf, sizeOfDoc, sizeOfVoc);
                    pwt.add(p);
    ...
    // Get exact count.
    private static int countExactTermFreqInList(String term, ArrayList<String> list) {
        int occurrences = Collections.frequency(list, term);
        return occurrences;
    }

    private static float laplaceSmoothing(int tf_given_term, int all_term_tf, int size_of_v) {
        float p = (float) (tf_given_term + 1) / (float) (all_term_tf + size_of_v);
        return p;
    }



    public static int getSizeOfVocabulary(ArrayList<String> unigramList)
    {
        ArrayList<String> list = new ArrayList<>();
        Set<String> hash = new HashSet<>();

        hash.addAll(unigramList);
        list.addAll(hash);
        return list.size();
    }
